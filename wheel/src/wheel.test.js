...
describe('wheel', () => {
  ...
  it('has multipliers for 10 wheel segments', () => {
    expect(Array.isArray(wheelMultipliers)).to.be.true;
    expect(wheelMultipliers).to.have.lengthOf(10);
  });
  ...
  it('used nonce in order', async () => {
    ...
    let bets = await Promise.all(
      _.range(5).map(() => spinWheel({ user, amount: 0.1 }))
    );
